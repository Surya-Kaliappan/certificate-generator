...
async function deleteBackgroundImage() {
  if (backgroundImageId) {
    try {
      const response = await fetch('http://localhost:3000/delete-background', {
        method: 'POST',
...
async function fetchFonts() {
  try {
    const response = await fetch('http://localhost:3000/fonts');
...
async function selectFont(fontName) {
  try {
    ...
    const response = await fetch(`http://localhost:3000/font/${fontName}`);
...
async function uploadBackgroundImage(file) {
  ...
  try {
    const response = await fetch('http://localhost:3000/upload-background', {
      method: 'POST',
      body: formData
    });
    if (!response.ok) {
...
async function checkEmailStatus(batchId) {
  try {
    const response = await fetch(`http://localhost:3000/email-status?batchId=${batchId}`);
    if (!response.ok) {
